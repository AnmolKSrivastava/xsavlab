...
const { escapeHtml, formatServiceName } = require('../lib/content');
...
if (!isEmulator) {
  sendEnquiryOptions.secrets = [EMAIL_USER, EMAIL_PASSWORD, ADMIN_EMAIL];
}
...
async function checkRateLimit(identifier) {
  ...
  return dailySubmissions.size >= 10;
}

const sendEnquiry = onRequest(sendEnquiryOptions, (req, res) => {
  cors(req, res, async () => {
    ...
    try {
      ...
      const bodySize = Buffer.byteLength(JSON.stringify(requestBody), 'utf8');
      if (bodySize > 10240) {
        return res.status(413).json({ error: 'Request payload too large' });
      }

      const { name, email, company, service, message } = requestBody;
      ...
      try {
        isRateLimited = await checkRateLimit(clientIp);
      } catch (rateLimitError) {
        // Fail-open in case index/config issues break the limiter.
        console.error('Rate limit check failed (continuing request):', rateLimitError);
      }
      ...
      const enquiryRef = await admin.firestore().collection('enquiries').add(enquiryData);
...
const getEnquiries = onRequest((req, res) => {
  cors(req, res, async () => {
